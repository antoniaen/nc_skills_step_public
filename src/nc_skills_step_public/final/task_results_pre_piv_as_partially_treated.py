...
The last untreated cohort is defined as partially treated (and treated).
...
for group in gl.groups_of_dependent_variables:
    kwargs = {
        ...
        "produces": {
            ...
            / "pre-pivotal_as_partially_treated"
            / f"results_pre_piv_as_partially_treated_{group}.tex",
            "pdf": BLD
            / "python"
            / "tables"
            / "with_partially_treated"
            / "pre-pivotal_as_partially_treated"
            / f"results_pre_piv_as_partially_treated_{group}.pdf",
            "tex_tabular": BLD
            / "python"
            / "tables"
            / "with_partially_treated"
            / "pre-pivotal_as_partially_treated"
            / f"results_pre_piv_as_partially_treated_{group}_tabular.tex",
        },
    }
    ...
    def task_latex_results_pre_piv_as_partially_treated(
        depends_on,
        group,
        produces,
    ):
        ...
        # Define pre-pivotal as partially treated.
        for data in (reg_data_3y, reg_data_5y, reg_data_10y):
            data["partially_treated"] = np.where(
                (data["rel_cohort"] == -1) & (data["country_reform"] != "Vietnam1991"),
                1,
                data["partially_treated"],
            )
            data["treated"] = np.where(
                (data["rel_cohort"] == -1) & (data["country_reform"] != "Vietnam1991"),
                1,
                data["treated"],
            )
