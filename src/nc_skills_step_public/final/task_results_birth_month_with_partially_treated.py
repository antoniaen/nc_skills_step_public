"""LaTeX tables with multiple regression results in one table.

Birth month instead of birth year as running variable. Partially treated_w_month
individuals are included.

"""

import pandas as pd
import pylatex as pl
import pytask

from nc_skills_step_public import global_info as gl
from nc_skills_step_public.analysis import analysis_RDD_w_month as reg
from nc_skills_step_public.analysis import select_sample_for_analysis as sel
from nc_skills_step_public.config import BLD, SRC
from nc_skills_step_public.final import latex_tables_with_regression_results as tab

for group in gl.groups_of_dependent_variables:
    kwargs = {
        "group": group,
        "produces": {
            "tex": BLD
            / "python"
            / "tables"
            / "with_partially_treated"
            / "with_birth_month"
            / f"results_month_with_partially_treated_{group}.tex",
            "pdf": BLD
            / "python"
            / "tables"
            / "with_partially_treated"
            / "with_birth_month"
            / f"results_month_with_partially_treated_{group}.pdf",
            "tex_tabular": BLD
            / "python"
            / "tables"
            / "with_partially_treated"
            / "with_birth_month"
            / f"results_month_with_partially_treated_{group}_tabular.tex",
        },
    }

    @pytask.mark.depends_on(
        {
            "scripts": ["latex_tables_with_regression_results.py"],
            "reg_functions": SRC / "analysis" / "analysis_RDD_w_month.py",
            "select_sample": SRC / "analysis" / "select_sample_for_analysis.py",
            "global_info": SRC / "global_info.py",
            "data": BLD / "python" / "data" / "step_reforms_final.pkl",
        },
    )
    @pytask.mark.task(id=group, kwargs=kwargs)
    def task_latex_regression_results_w_month_w_partially_treated(
        depends_on,
        group,
        produces,
    ):
        """Latex tabular with regression results."""
        dep_vars = gl.groups_of_dependent_variables[group]
        results_dict = {key: None for key in dep_vars}
        dep_var_names = {key: gl.nice_variable_names[key] for key in dep_vars}

        data = pd.read_pickle(depends_on["data"])

        # 3 years
        reg_data_3y = sel.select_sample_for_analysis_months_based(
            data=data,
            y_vars=dep_vars,
            n_months=3 * 12,
            reform_list=gl.reforms_final,
        )

        # 5 years
        reg_data_5y = sel.select_sample_for_analysis_months_based(
            data=data,
            y_vars=dep_vars,
            n_months=5 * 12,
            reform_list=gl.reforms_final,
        )
        # 10 years
        reg_data_10y = sel.select_sample_for_analysis_months_based(
            data=data,
            y_vars=dep_vars,
            n_months=10 * 12,
            reform_list=gl.reforms_final,
        )

        for y_var in dep_vars:
            results_3inf = reg.linear_inflexible_trends_w_month(
                data=reg_data_3y,
                y_var=y_var,
                partially_treated=True,
                partially_treated_trend=False,
            )
            results_3flex = reg.linear_flexible_trends_w_month(
                data=reg_data_3y,
                y_var=y_var,
                partially_treated=True,
                partially_treated_trend=False,
            )
            results_5inf = reg.linear_inflexible_trends_w_month(
                data=reg_data_5y,
                y_var=y_var,
                partially_treated=True,
                partially_treated_trend=False,
            )
            results_5flex = reg.linear_flexible_trends_w_month(
                data=reg_data_5y,
                y_var=y_var,
                partially_treated=True,
                partially_treated_trend=False,
            )
            results_10inf = reg.linear_inflexible_trends_w_month(
                data=reg_data_10y,
                y_var=y_var,
                partially_treated=True,
                partially_treated_trend=False,
            )
            results_10flex = reg.linear_flexible_trends_w_month(
                data=reg_data_10y,
                y_var=y_var,
                partially_treated=True,
                partially_treated_trend=False,
            )
            results_10quad = reg.quadratic_flexible_trends_w_month(
                data=reg_data_10y,
                y_var=y_var,
                partially_treated=True,
                partially_treated_trend=False,
            )

            results_dict[y_var] = [
                results_10inf,
                results_10flex,
                results_10quad,
                results_3inf,
                results_3flex,
                results_5inf,
                results_5flex,
            ]

        column_headers = [
            "inflex.",
            "flex.",
            "quad.",
            "inflex.",
            "flex.",
            "inflex.",
            "flex.",
        ]

        # A table showing the regressors instead of the dependent variables in the first column.
        if group == "years_educ":
            tab.create_tabular_tex_code_reg_results_show_regressors(
                file=str(produces["tex_tabular"]).replace(".tex", ""),
                results=results_dict,
                regressor="treated_w_month",
                column_headers=column_headers,
                reg_var_names={
                    "treated_w_month": "Treated",
                    "partially_treated": pl.NoEscape(
                        r"Treated $\times$ Partially treated",
                    ),
                },
                additional_regressor="partially_treated",
                version=2,
            )

            tab.create_tabular_tex_code_with_reg_results(
                file=str(produces["tex"]).replace(".tex", ""),
                results=results_dict,
                regressor="treated_w_month",
                column_headers=column_headers,
                dep_var_names=dep_var_names,
                version=2,
                gen_pdf=True,
            )

        else:
            tab.create_tabular_tex_code_with_reg_results(
                file=str(produces["tex"]).replace(".tex", ""),
                results=results_dict,
                regressor="treated_w_month",
                column_headers=column_headers,
                dep_var_names=dep_var_names,
                version=2,
                gen_pdf=True,
            )

            tab.create_tabular_tex_code_with_reg_results(
                file=str(produces["tex_tabular"]).replace(".tex", ""),
                results=results_dict,
                regressor="treated_w_month",
                column_headers=column_headers,
                dep_var_names=dep_var_names,
                version=2,
                gen_pdf=False,
            )
